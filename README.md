...
# 画面
 - 初期画面
   ![初期画面](screenshots/screenshot20260119-main01.png)

 - 分割したファイルを変換した後
   ![分割したファイルを変換した後](screenshots/screenshot20260119-main02.png)

 - 出力されたQRコード
   ![出力されたQRコード](screenshots/screenshot20260119-main_result.png)

 - 画像生成
   ![画像生成](screenshots/screenshot20260119-createimage.png)

 - 設定
   ![設定](screenshots/screenshot20260119-setting.png)
...
3. 変換対象リストのエラー補正レベル(ECL)を選択します。
   L→M→Q→Hの順でQRコードの汚損に強くなりますが、同じサイズのQRコードに格納可能な文字数が少なくなります。
...
5. \[要素位置\]を入力します。  
   ...
   省略した場合、4pixel角の黒四角で画像を生成します。
     ...
     - 要素位置\[画像読込\]ボタンからシートに画像を読み込むことができます。  
       一応、BMP以外の画像も読み込めますが、変換が完全ではありません。
6. \[背景色\]をRGB順の16進数で入力します。  
   省略した場合、白色(FFFFFF)が使用されます。
   濃すぎる場合、QRコードリーダーが正常に読み取れないでしょう。
   背景色\[選択\]ボタンで色を選択可能です。
7. \[変換\]ボタンを押下します。
...
- 解析のため、大きなサブルーチンを分解しました。ローカル変数の利用範囲を限定したかったので。
     - QR_genの大部分を以下のサブルーチンに分解しました。
       |サブルーチン名  |機能                    |元の記述位置                            |
       |----------------|------------------------|----------------------------------------|
       |QR_anlyz        |文字列解析              |QR_paramsの呼び出しの手前まで。<br/>※文字列解析においてecx_pocに加算している部分と、eb(i, 4)に格納してから変数cに合算している部分はビット長計算部分としてQR_search_paramsに移動しました。|
...
- 1bit、4bit、8bit、24bit形式の無圧縮形式のBMPファイルを生成します。
...
- 1bit、4bit、8bit、24bit形式の無圧縮形式のBMPファイルを読み込みます。  
 出力は24bitカラーマップのみです。